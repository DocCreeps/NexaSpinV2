...
<div {{ $attributes->class(['relative w-full max-w-[320px] mx-auto']) }} x-data="
    {
        rotation:0,
        spinning:false
    }" x-on:{{ $event }}.window="
    ...
    <svg viewBox="0 0 300 300" class="w-full drop-shadow-lg">
        ...
        <g x-bind:style="
            ...
            transition:
            transform 4.5s cubic-bezier(.17,.67,.2,1);
            `
            ">
            ...
            <text transform="{{ $segment['labelTransform'] }}" text-anchor="middle" class="fill-white text-[11px] font-medium" style="
                        paint-order:stroke;
